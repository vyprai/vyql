...
func (c *ktConv) expr(n *tree_sitter.Node) nir.Expr {
	...
	case "unary_expression":
		// One node kind carries prefix (`-x`, `!x`, `++x`) and postfix
		// (`x!!`, `x++`, `x--`) operators alike; the operator's position
		// relative to the argument tells them apart. A postfix operator does
		...
		if op != nil && arg != nil && op.StartByte() < arg.StartByte() {
			return nir.Unary{Op: c.text(op), Operand: operand, Loc: L}
		}
		return nir.Thru{Inner: operand}
	case "if_expression":
