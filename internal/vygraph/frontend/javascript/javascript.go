// Package javascript is the v3 JavaScript/TypeScript-syntax frontend: source
// lowered into the closed NIR graph, same contract as the python reference
// frontend (see ENGINE-FORK.md). Structure and Region/Order only; the shared
// lowering computes FLOWS and resolution.
package javascript
...
func (c *conv) add(n *tree_sitter.Node, typ string, fields [][2]string, approx bool) (graph.Node, bool) {
	...
	node := graph.Node{ID: id, Type: typ, Layer: graph.LayerLow, Fields: flds,
		Prov: graph.Provenance{Producer: "javascript", Build: graph.BuildParsed, Trust: graph.TrustTrusted}}
...
func (c *conv) collectImports(root *tree_sitter.Node, imp Imports) {
	...
	walk = func(n *tree_sitter.Node) {
		...
		if n.Kind() == "import_statement" {
			spec := ""
			// The specifier string may follow the clause; find it first.
			for i := 0; i < int(n.NamedChildCount()); i++ {
				if d := n.NamedChild(uint(i)); d != nil && d.Kind() == "string" {
					spec = strings.Trim(c.text(d), `'"`)
				}
			}
			for i := 0; i < int(n.NamedChildCount()); i++ {
				...
				case "import_clause":
					for j := 0; j < int(d.NamedChildCount()); j++ {
						...
						case "identifier":
							if spec != "" {
								imp.Modules[c.text(e)] = spec
							}
						case "named_imports":
							for k := 0; k < int(e.NamedChildCount()); k++ {
								if ni := e.NamedChild(uint(k)); ni != nil && ni.Kind() == "import_specifier" {
									name := ""
									for m := 0; m < int(ni.NamedChildCount()); m++ {
										if s := ni.NamedChild(uint(m)); s != nil {
											name = c.text(s)
										}
									}
									if spec != "" && name != "" {
										imp.Modules[name] = spec
									}
								}
							}
						}
	...
	case "if_statement":
		c.structured(n, "if", func(outer string) {
			for i := 0; i < int(n.NamedChildCount()); i++ {
				...
				case "else_clause":
					for j := 0; j < int(d.NamedChildCount()); j++ {
						if e := d.NamedChild(uint(j)); e.Kind() == "statement_block" {
							c.in(outer+"/else", func() { c.stmts(e) })
						}
					}
				}
			}
...
func (c *conv) expr(n *tree_sitter.Node) graph.Node {
	...
	case "function_expression":
		...
		c.functionDef(n, name)
		return c.mustAdd(n, "code.FuncDef", [][2]string{{"name", name}, {"qualified_name", c.file}}, false)
	case "arrow_function":
		// Arrow callbacks are functions: their bodies are walked (the handler
		// content of a registration call lives here), with a Lambda node
		// marking the anonymous shape.
		name := fmt.Sprintf("arrow#%d", n.StartPosition().Row+1)
		c.functionDef(n, name)
		return c.mustAdd(n, "code.Lambda", [][2]string{{"kind", "arrow"}}, false)
	case "ternary_expression":
		return c.mustAdd(n, "code.Ternary", nil, true)
	case "parenthesized_expression", "await_expression", "spread_element", "argument_list", "expression_statement":
		for i := 0; i < int(n.NamedChildCount()); i++ {
			if d := n.NamedChild(uint(i)); d != nil {
				return c.expr(d)
			}
...
func (c *conv) template(n *tree_sitter.Node) graph.Node {
	node := c.mustAdd(n, "code.Format", [][2]string{{"text", c.text(n)}}, false)
