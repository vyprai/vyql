...
// conceptSnake turns "code.HttpInput" into "http_input" for the best-effort source_type.
...
// sinkOps maps sink concepts to VyPr SinkType enum values (verbatim passthrough).
var sinkOps = map[string]string{
	"CommandExecution":       "command_exec",
	"CodeEval":               "code_eval",
	"ExpressionEval":         "code_eval",
	"InteractiveInterpreter": "code_eval",
	"ReflectionInvoke":       "code_eval",
	"SqlExecution":           "sql_query",
	"DatabaseRead":           "database_read",
	"NosqlQuery":             "nosql_query",
	"TemplateRender":         "template_render",
	"HtmlRender":             "html_output",
	"UrlFetch":               "http_request",
	"RedirectTarget":         "redirect",
	"ResponseHeaderWrite":    "header_injection",
	"MailHeaderWrite":        "header_injection",
	"Deserialization":        "deserialization",
	"XmlParse":               "xml_parse",
	"LdapQuery":              "ldap_query",
	"XpathQuery":             "xpath_query",
	"RegexCompile":           "regex_eval",
	"FormatString":           "format_string",
	"ProtoPollute":           "prototype_pollution",
	"WeakHash":               "weak_hash",
	"WeakCipher":             "crypto_op",
	"WeakRandomValue":        "weak_random",
	"LogOutput":              "log_write",
	"LogWrite":               "log_write",
	"InsecureCookie":         "insecure_cookie",
	"TrustBoundary":          "trust_boundary",
	"SecretValue":            "secret_access",
	"HardcodedSecret":        "credential_leak",
	"FilePathAccess":         "file_read", // direction best-effort; see schema wrinkle #2
	"FileInclude":            "file_read",
	"UnsafeUpload":           "file_write",
}

// sourceConcepts are request/input origins (role=source in the legend).
var sourceConcepts = map[string]bool{
	"HttpInput": true, "DomInput": true, "StdinInput": true, "EnvVariable": true,
	"CliArgument": true, "IntentInput": true, "URLSchemeInput": true, "PasteboardInput": true,
}

// allConcepts is the canonical concept inventory (kept in sync with the adapters/packs).
var allConcepts = []string{
	"CertValidationDisabled", "CleartextChannel", "CleartextConfig", "CliArgument",
	"CodeEval", "CommandExecution", "CsrfDisabled", "CsvCell", "DatabaseRead",
	"DebuggableApp", "Deserialization", "DomInput", "ElectronNodeIntegration",
	"EnvVariable", "ExportedComponent", "ExpressionEval", "ExternalCall", "FileInclude",
	"FilePathAccess", "FormatString", "HardcodedSecret", "HostNamespaceShare", "HtmlRender",
	"HttpInput", "InsecureCookie", "InsecurePermission", "InsecureTempFile", "IntentInput",
	"InteractiveInterpreter", "JwtUnverified", "LdapQuery", "LogOutput", "LogWrite",
	"MailHeaderWrite", "MassAssign", "MissingFrameProtection", "NosqlQuery", "PasteboardInput",
	"PermissiveCors", "PiiValue", "PrivilegedContainer", "PrivilegedOp", "ProtoPollute",
	"PublicNetworkExposure", "RedirectTarget", "ReflectionInvoke", "RegexCompile",
	"ResourceHandle", "ResponseHeaderWrite", "SecretValue", "SessionId", "SqlExecution",
	"StateChangingOp", "StateWrite", "StdinInput", "TemplateRender", "TrustBoundary",
	"TxOriginAuth", "URLSchemeInput", "UnsafeUpload", "UrlFetch", "WeakCipher", "WeakHash",
	"WeakRandomValue", "WebViewJsInterface", "WebViewLoad", "WildcardIamPolicy", "XmlParse",
	"XpathQuery",
}

func conceptLegend() []gjConcept {
	out := make([]gjConcept, 0, len(allConcepts))
	for _, c := range allConcepts {
		gc := gjConcept{Concept: "code." + c, Role: "presence"}
		if op, ok := sinkOps[c]; ok {
			gc.Role = "sink"
			gc.Operation = &op
		} else if sourceConcepts[c] {
			gc.Role = "source"
		}
		out = append(out, gc)
	}
